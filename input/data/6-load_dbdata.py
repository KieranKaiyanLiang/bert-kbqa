...
def create_db():
    ...
    #                          创建表
    conn.execute("drop database if exists KBQA")  # 如果new_database数据库存在则删除
    conn.execute("create database KBQA")  # 新创建一个数据库
    conn.execute("use KBQA")  # 选择new_database这个数据库
...
def loaddata():
    ...
    #    'mysql+pymysql://%root")s:%("123456")s@%("0.0.0.0")s:%(3306)d/%("KBQA)s?charset=utf8' % db_info, encoding='utf-8')
    ...
    SQLALCHEMY_DATABASE_URI='mysql+pymysql://root:123456@0.0.0.0:3306/KBQA?charset=utf8'
    ...
    df.to_sql('nlpccqa', con=engine,  if_exists='replace')#这种形式也可以
