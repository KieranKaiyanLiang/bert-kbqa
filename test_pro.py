...
# attribute_idx = semantic_matching(sim_model, tokenizer, raw_text, attribute_list, answer_list, 64).item()
    ...
    for (ex_index, attribute) in enumerate(attribute_list):
        if attribute_list[ex_index]=="111":
            continue
        inputs = tokenizer.encode_plus(
            text = question,
            text_pair = attribute,
...
def select_database(sql):
    # connect database
    connect = pymysql.connect(user="root",password="123456",host="0.0.0.0",port=3306,db="KBQA",charset="utf8")
...
def main():
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            # print(triple_list)
            attribute_list = list(triple_list[2])
            answer_list = list(triple_list[3])
            ...
            if attribute != '':
                for i in attribute:
                    ret += "{}的{}是{}\n".format(entity, attribute_list[i], answer_list[i])
                    raw_text=raw_text.replace(attribute_list[i],"")

                attribute_list = [elem for i, elem in enumerate(attribute_list) if i not in attribute]
                answer_list = [elem for i, elem in enumerate(answer_list) if i not in attribute]
            ...
            if -1 == attribute_idx:
                ...
                ret += "{}的{}是{}\n".format(entity, attribute, answer)
            if '' == ret:
                print("关于{}，或许你想查询：".format(entity))
